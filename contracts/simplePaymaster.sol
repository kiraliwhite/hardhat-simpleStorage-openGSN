...
contract simplePaymaster is BasePaymaster {
  ...
  function _preRelayedCall(
    ...
    //approvalData是由 Web 客戶端通過中繼器relay發送的。它可以包含 DApp 需要的任何數據，以判斷是否批准請求。請注意，默認情況下，BasePaymaster 將拒絕帶有apporvalData（或paymasterData）的請求。
    ...
    )
  {
    ...
    //返回值用於從 preRelayedCall（接收整個請求）傳遞“context上下文、脈絡、情境”給 postRelayedCall。對於一個簡單的paymaster（像我們的...），這是不需要的，但是更複雜的支付主機可能會執行一些後置中繼操作。在這個示例中，可以返回空值。
